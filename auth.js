...
// FIX 1: Redirect on BOTH SIGNED_IN and INITIAL_SESSION
_supabase.auth.onAuthStateChange(async (event, session) => {
  console.log("AUTH EVENT:", event, session);
  
  if ((event === 'SIGNED_IN' || event === 'INITIAL_SESSION') && session) {
    console.log("Session found, saving to DB...");
...
// FIX 2: Removed the broken setTimeout check entirely
// onAuthStateChange handles everything now
