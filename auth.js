...
// CRITICAL: Only redirect after we get SIGNED_IN, not on page load
_supabase.auth.onAuthStateChange(async (event, session) => {
  console.log("AUTH EVENT:", event);
  if (event === 'SIGNED_IN' && session) {
    console.log("User signed in, saving to DB...");
...
// Let detectSessionInUrl finish first
setTimeout(async () => {
  const { data: { session } } = await _supabase.auth.getSession();
  console.log("SESSION ON LOAD:", session);
  if (session && !window.location.href.includes('#access_token')) {
    window.location.replace('Report.html');
  }
}, 800);
...
document.getElementById('detailsForm').onsubmit = async (e) => {
  ...
  const { error } = await _supabase.auth.signInWithOtp({
    email: userEmail,
    options: {
      emailRedirectTo: 'https://zotry.vercel.app', // NO trailing slash
