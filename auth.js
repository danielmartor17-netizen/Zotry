...
// NEW: Handle magic link + check existing session
_supabase.auth.onAuthStateChange((event, session) => {
  if (session && (event === 'SIGNED_IN' || event === 'INITIAL_SESSION')) {
    window.location.href = 'Report.html'; // Change to 'home.html' if that's your dashboard
  }
});

// Also check on page load for existing session
_supabase.auth.getSession().then(({ data: { session } }) => { 
  if (session) window.location.href = 'Report.html'; // Change to 'home.html' if needed
}); 
...
document.getElementById('otpForm').onsubmit = async (e) => { 
  ...
  if (error) { 
    ...
    window.location.href = 'Report.html'; // Change to 'home.html' if needed
  } 
};